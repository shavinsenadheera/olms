...
<body>
	...
	<div id="maindiv">
		...
		<?php	
			...
			{
				$sql = "SELECT issue.id ,issue.issuedate,issue.status,book.title FROM issue ,book WHERE issue.registerNo = '$_SESSION[srg]' AND book.id = issue.id";
				...
				if($result->num_rows>0)
				{
					while($row = $result->fetch_assoc())
					{
						...
						echo "<tr>
							...
							<td>Issue Date</td><td>".$row['issuedate']."</td></tr>
				...
				else
				{
					?>
					<br>
					<span id="nonew">No Borrow Books</span>
					<?php
				}
			$connection->close();
		}
