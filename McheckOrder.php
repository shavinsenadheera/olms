...
<title>LMS - Checking Orders</title>
</head>
...
<body>
	...
	<div id="maindiv">
		...
		<?php	
			...
			{
				$sql = "SELECT orderbook.id ,orderbook.orderdate,orderbook.status , book.title FROM orderbook ,book WHERE orderbook.registerNo = '$_SESSION[srg]' AND book.id = orderbook.id";
				...
				if($result->num_rows>0)
				{
					while($row = $result->fetch_assoc())
					{
						...
						echo "<tr>
							...
							<td>Order Date</td><td>".$row['orderdate']."</td></tr>
							<tr><td>Current Status</td><td>".$row['status']."</td></tr></table>
							<p>";
					}
					?>
					<?php
				...
				else
				{
					?>
					<br>
					<span id="nonew">Not Available Books</span>
					<?php
				}
			$connection->close();
		}
