...
<body>
	...
	<div id="maindiv">
		...
		<?php
			...
			{
				$sql = "SELECT * FROM member WHERE registerNo = '$_SESSION[srg]'";
				...
				if($result->num_rows>0)
				{
					while($row = $result->fetch_assoc())
					{
						...
						echo "<tr>
							...
							<tr>
								<td id='tdEmpty'></td>
								<td id='tdLink'><a href='MprofileChange1.php?ctry=$row[category]&rgno=$row[registerNo]&fname=$row[firstName]&mname=$row[middleName]&lname=$row[lastName]&adrs=$row[address]&gndr=$row[gender]&eml=$row[email]&cntno=$row[contactNo]&uname=$row[username]&pwd=$row[password]'>Change Details</a></td>
							</tr>
							</table></form>
							<p>";
					}
				...
				{
					print("You'r profile doesn't appear. Please contact Administrator.");
				}
			$connection->close();
		}
		?>
